...
public class Freecam extends Module {
    ...
    private final Setting<Double> speed = sgGeneral.add(new DoubleSetting.Builder()
        .name("speed").description("Camera fly speed (blocks per tick).").defaultValue(1.0).min(0.05).sliderRange(0.1, 5.0).build());
    ...
    private final Setting<Double> verticalSpeed = sgGeneral.add(new DoubleSetting.Builder()
        ...
        .defaultValue(1.0).min(0.5).max(4).sliderRange(0.5, 4).decimalPlaces(2).build());
    private final Setting<Double> smoothing = sgGeneral.add(new DoubleSetting.Builder()
        .name("smoothing").description("How much the camera eases into a start and out of a stop. A little takes the jerkiness off without making it feel like it is floating; 0 is instant, the way Meteor moves.").defaultValue(0.12).min(0).max(0.9).sliderRange(0, 0.5).decimalPlaces(2).build());
    ...
    private void moveCamera() {
        ...
        if (phys(mc.options.jumpKey)) target = target.add(0, 1, 0);
        if (phys(mc.options.sneakKey)) target = target.add(0, -1, 0);
        if (target.lengthSquared() > 0) target = target.normalize();
        target = target.multiply(s);
        // rise and fall get their own multiplier, applied after the direction is settled so it
        // does not skew which way you are actually heading
        if (verticalSpeed.get() != 1.0)
            target = new Vec3d(target.x, target.y * verticalSpeed.get(), target.z);
